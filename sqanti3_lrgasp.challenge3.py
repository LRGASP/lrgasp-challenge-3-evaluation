...
def get_corrBam_from_corrSam(sam):
    base_name=os.path.splitext(sam)[0]
    corrBam=base_name + ".bam"
    os.system('samtools view -b -h '+ sam + ' > ' + corrBam)
    return corrBam
...
def run_BUSCO(correctedFASTA, cpus, output_dir):
    ...
    if os.path.exists(summary_file):
        ...
        subprocess.call(['busco','-i',str(correctedFASTA), '-l', 'eutheria_odb10', '--out_path', output_dir, '-o','BUSCO_analysis', '-m', 'transcriptome', '-c', str(cpus)])
    summary = open(summary_file,"r")
    table = {}
    for line in summary:
        if "Complete and single-copy BUSCOs" in line:
            table["Complete and single-copy BUSCOs"] = int(line.split("\t")[1])
        elif "Complete and duplicated BUSCOs" in line:
            table["Complete and duplicated BUSCOs"] = int(line.split("\t")[1])
        elif "Fragmented BUSCOs" in line:
            table["Fragmented BUSCOs"] = int(line.split("\t")[1])
        elif "Missing BUSCOs" in line:
            table["Missing BUSCOs"] = int(line.split("\t")[1])
    results_tsv = output_dir + "/BUSCO_results.tsv"
    with open(results_tsv, "w") as results:
        dictwritr = DictWriter(results, table.keys(), delimiter='\t')
        dictwritr.writeheader()
        dictwritr.writerow(table)
    summary.close()
    return results_tsv
